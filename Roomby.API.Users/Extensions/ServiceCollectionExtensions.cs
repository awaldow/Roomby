...
using Roomby.API.Users.Infrastructure.Filters;
...
namespace Roomby.API.Users.Extensions
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddUsersMVC(this IServiceCollection services, IConfiguration configuration)
        {
        ...
        public static IServiceCollection AddCustomHealthCheck(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            hcBuilder.AddCheck("self", () => HealthCheckResult.Healthy())
                    .AddSqlServer(
                            connectionString: configuration.GetConnectionString("RoombyUsersSql"),
                            name: "usersDB-check",
        ...
        public static IServiceCollection AddUsersDbContext(this IServiceCollection services, IConfiguration configuration, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                services.AddDbContext<UsersContext>(options =>
                {
                    options.UseSqlServer(configuration.GetConnectionString("mssql-users"), sqlServerOptionsAction: sqlOptions =>
                    {
            ...
            else
            {
                services.AddDbContext<UsersContext>(options =>
                {
                    options.UseSqlServer(configuration.GetConnectionString("RoombyUsersSql"), sqlServerOptionsAction: sqlOptions =>
                    {
                        sqlOptions.MigrationsAssembly(typeof(Startup).GetTypeInfo().Assembly.GetName().Name);
                        sqlOptions.EnableRetryOnFailure(maxRetryCount: 10, maxRetryDelay: TimeSpan.FromSeconds(30), errorNumbersToAdd: null);
                    });
